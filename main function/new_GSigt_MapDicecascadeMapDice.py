...
def train_valid_seg(episode,config):
    ...
    if config['if_test']:
        ...
        test_diceLoss, test_mapDice, test_loss, test_dice1, test_dice2, test_iter  = test(test_loader, net, net_tail, diceLoss, mapDiceLoss, config['saved_dir'])
        ...
        print("test_dice:%.4f, test_mapDice:%.4f, test_loss:%.4f, test_dice1:%.4f, test_dice2:%.4f, Time:%.3fmin" %
              (test_avgdiceloss, test_avgmapDice, test_avgloss, test_avgdice1, test_avgdice2, (time.time() - start_time) / 60))
        return
    ...
    for epoch in range(config['max_epoches']):
        for epi in range(episode):
            ...
            train_diceLoss, train_mapDice, train_loss, train_dice1, train_dice2, train_iter = train(train_loader, net, net_tail, diceLoss, mapDiceLoss, optimizer)
            ...
            print("[%d-%d/%d], train_dice:%.4f, train_mapDice:%.4f, train_loss:%.4f, train_dice1:%.4f, train_dice2:%.4f, Time:%.3fmin" %
                  (epoch, epi, config['max_epoches']-1, train_avgdiceLoss, train_avgmapDice, train_avgloss, train_avgdice1, train_avgdice2,(time.time() - start_time) / 60))

            print('######valid epoch-epi', str(epoch),'-',str(epi),'######')
            valid_diceLoss, valid_mapDice, valid_loss, valid_dice1, valid_dice2, valid_iter = validate(val_loader, net, net_tail, diceLoss, mapDiceLoss)
            ...
            print("[%d-%d/%d], valid_dice:%.4f, valid_mapDice:%.4f, valid_loss:%.4f, valid_dice1:%.4f, valid_dice2:%.4f,Time:%.3fmin " %
                  (epoch, epi, config['max_epoches'], valid_avgdiceLoss, valid_avgmapDice, valid_avgloss, valid_avgdice1, valid_avgdice2,(time.time() - start_time) / 60))
...
def train(data_loader, net, net_tail, diceLoss, mapDiceLoss, optimizer):
    ...
    for i, (data,target,target_map,origin,direction,space,prefix,subNo) in enumerate(data_loader):
        ...
        output1 = net(data)
        loss_dice = diceLoss(output1, target_map)
        ...
        print("[%d/%d], loss_dice:%.4f, loss_mapDice:%.4f, loss:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
              (i, total_iter, loss_dice.item(), loss_mapDice.item(), loss.item(), dice1, dice2, name))
    return epoch_diceLoss, epoch_mapDice, epoch_loss, epoch_dice1, epoch_dice2, total_iter

def validate(data_loader, net, net_tail, diceLoss, mapDiceLoss):
    net.eval()
    epoch_diceLoss = []
    epoch_mapDice = []
    epoch_loss = []
...
def test(data_loader, net, net_tail, diceLoss, mapDiceLoss, saved_dir):
    net.eval()
    epoch_diceLoss = []
    epoch_mapDice = []
    epoch_loss = []
    ...
    with torch.no_grad():  # no backward
        for i, (data, target, target_map,origin, direction, space, prefix, subNo) in enumerate(data_loader):
            ...
            name = [prefix[i].split('-')[-1] + '/' + subNo[i] for i in range(num)]
            print(
                "[%d/%d], loss_dice:%.4f, loss_mapDice:%.4f, loss:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
                (i, total_iter, loss_dice.item(), loss_mapDice.item(), loss.item(), dice1, dice2, name))
...
if __name__ == '__main__':
    ...
    viz = Visdom(env='G6 TumorNet-MapDice cascade frozen-MapNet-mapDice')
    viz.line([0], [0], win='tloss',opts=dict(title='train avgloss', legend=['dice', 'mapDice', 'loss']))
    viz.line([0], [0], win='tdice', opts=dict(title='train avgdice', legend=['dice1', 'dice2']))
    viz.line([0], [0], win='vloss',opts=dict(title='valid avgloss', legend=['dice', 'mapDice', 'loss']))
    ...
    config = {
        ...
        'ckpt_dir': './G00results/',
        'saved_dir': "/data/lihuiyu/LiTS/Valid_results/check822MapDiceL1N/",#"/data/lihuiyu/LiTS/Valid_results/checkDiceCosine0/",
    ...
    from TumorNet import MapDiceLoss as MapDiceLoss1
