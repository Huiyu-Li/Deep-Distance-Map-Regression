...
def train_valid_seg(episode,config):
    ...
    if config['if_test']:
        ...
        test_smoothL1, test_mapDice, test_real, test_loss, test_dice1, test_dice2, test_iter  = test(test_loader, net, net_tail, smoothL1, mapDiceLoss, config['saved_dir'])
        test_avgsmoothL1 = sum(test_smoothL1, ) / test_iter
        test_avgmapDice = sum(test_mapDice) / test_iter
        test_avgreal = sum(test_real) / test_iter
        test_avgloss = sum(test_loss) / test_iter
        test_avgdice1 = sum(test_dice1) / test_iter
        test_avgdice2 = sum(test_dice2) / test_iter
        print("test_smoothL1:%.4f, test_mapDice:%.4f, test_real:%.4f, test_loss:%.4f, test_dice1:%.4f, test_dice2:%.4f, Time:%.3fmin" %
              (test_avgsmoothL1, test_avgmapDice, test_avgreal, test_avgloss, test_avgdice1, test_avgdice2, (time.time() - start_time) / 60))
        return
    ...
    for epoch in range(config['max_epoches']):
        for epi in range(episode):
            ...
            train_smoothL1, train_mapDice, train_real, train_loss, train_dice1, train_dice2, train_iter = train(train_loader, net, net_tail, smoothL1, mapDiceLoss, optimizer)
            train_avgsmoothL1 = sum(train_smoothL1) / train_iter
            train_avgmapDice = sum(train_mapDice) / train_iter
            train_avgreal = sum(train_real) / train_iter
            ...
            print("[%d-%d/%d], train_smoothL1:%.4f, train_mapDice:%.4f, train_real:%.4f, train_loss:%.4f, train_dice1:%.4f, train_dice2:%.4f, Time:%.3fmin" %
                  (epoch, epi, config['max_epoches']-1, train_avgsmoothL1, train_avgmapDice, train_avgreal, train_avgloss, train_avgdice1, train_avgdice2,(time.time() - start_time) / 60))

            print('######valid epoch-epi', str(epoch),'-',str(epi),'######')
            valid_smoothL1, valid_mapDice, valid_real, valid_loss, valid_dice1, valid_dice2, valid_iter = validate(val_loader, net, net_tail, smoothL1, mapDiceLoss)
            valid_avgsmoothL1 = sum(valid_smoothL1) / valid_iter
            valid_avgmapDice = sum(valid_mapDice) / valid_iter
            valid_avgreal = sum(valid_real) / valid_iter
            ...
            print("[%d-%d/%d], valid_smoothL1:%.4f, valid_mapDice:%.4f, valid_real:%.4f, valid_loss:%.4f, valid_dice1:%.4f, valid_dice2:%.4f,Time:%.3fmin " %
                  (epoch, epi, config['max_epoches'], valid_avgsmoothL1, valid_avgmapDice, valid_avgreal, valid_avgloss, valid_avgdice1, valid_avgdice2,(time.time() - start_time) / 60))
            ...
            viz.line([train_avgreal], [epoch * episode + epi], win='tloss', name='real', update='append')
            ...
            viz.line([valid_avgreal], [epoch * episode + epi], win='vloss', name='real', update='append')
...
def train(data_loader, net, net_tail, smoothL1, mapDiceLoss, optimizer):
    ...
    for i, (data,target,target_map,origin,direction,space,prefix,subNo) in enumerate(data_loader):
        ...
        print("[%d/%d], loss_smoothL1:%.4f, loss_mapDice:%.4f, loss_real:%.4f, loss:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
              (i, total_iter, loss_smoothL1.item(), loss_mapDice.item(), loss_real.item(), loss.item(), dice1, dice2, name))
    return epoch_smoothL1, epoch_mapDice, epoch_real, epoch_loss, epoch_dice1, epoch_dice2, total_iter

def validate(data_loader, net, net_tail, smoothL1, mapDiceLoss):
    net.eval()
    epoch_smoothL1 = []
    epoch_mapDice = []
    epoch_real = []
...
def test(data_loader, net, net_tail, smoothL1, mapDiceLoss, saved_dir):
    net.eval()
    epoch_smoothL1 = []
    epoch_mapDice = []
    epoch_real = []
    ...
    with torch.no_grad():  # no backward
        for i, (data, target, target_map, origin, direction, space, prefix, subNo) in enumerate(data_loader):
            ...
            name = [prefix[i].split('-')[-1] + '/' + subNo[i] for i in range(num)]
            print(
                "[%d/%d], loss_smoothL1:%.4f, loss_mapDice:%.4f, loss_real:%.4f, loss:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
                (i, total_iter, loss_smoothL1.item(), loss_mapDice.item(), loss_real.item(), loss.item(), dice1, dice2, name))
            ...
            plt.subplot(323);plt.imshow(output1[0, 0, slice, :, :].detach().cpu().numpy());plt.title('output1-0'+str(loss_smoothL1.item())[:4]);plt.axis('off');plt.colorbar()
...
if __name__ == '__main__':
    ...
    viz = Visdom(env='G4 TumorNet cascade frozenMapNet-L1+mapDice')
    viz.line([0], [0], win='tloss',opts=dict(title='train avgloss',legend = ['smoothL1','mapDice','real','loss']))
    viz.line([0], [0], win='tdice', opts = dict(title='train avgdice',legend = ['dice1','dice2']))
    viz.line([0], [0], win='vloss',opts=dict(title='valid avgloss', legend=['smoothL1', 'mapDice','real', 'loss']))
    ...
    config_splitData = {
        ...
        'savedmap_path': "/data/lihuiyu/LiTS/Preprocessed_S3_W20040_48/NormInverse_1/",
        'TVTcsv' : './G0TVTcsv/',
        'valid_csv' : './valid.csv',
        'episode' : 1,
        'ratio' : 0.9
    ...
    config = {
        ...
        'weight_decay' : 0,
        'train_csv_list': ['./G0TVTcsv/train' + str(i) + '.csv' for i in range(episode)],
        'valid_csv': './G0TVTcsv/valid.csv',
        'test_csv': './test.csv',
        # below is the saved path
        'ckpt_dir': './G00results/',
        'saved_dir': "/data/lihuiyu/LiTS/Valid_results/check822L1MapDiceN",
        ...
        'tail_dir':"/home/lihuiyu/Code/LiTS_TumorNet_Map/STOATumorMapNet/NormInverseMapNet/20191226-071848NormIeverse_1/model_78-0-1725.pth"
    ...
    from TumorNet import TumorNet, Dice
    from MapNet import *
