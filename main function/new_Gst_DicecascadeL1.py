...
def test(data_loader, net, net_tail, diceLoss, smoothL1, saved_dir):
    ...
    with torch.no_grad():  # no backward
        for i, (data, target, target_map,origin, direction, space, prefix, subNo) in enumerate(data_loader):
            ...
            plt.subplot(423);plt.imshow(output1[0, 0, slice, :, :].detach().cpu().numpy());plt.title('output1-0'+str(loss_dice.item())[:4]);plt.axis('off');plt.colorbar()
...
if __name__ == '__main__':
	...
	viz = Visdom(env='G3 TumorNet-Dice cascade frozen-MapNet-SmoothL1*10')
	...
	config = {
		...
		'ckpt_dir': './G00results/',
		'saved_dir': "",#"/data/lihuiyu/LiTS/Valid_results/checkDiceCosine0/",
