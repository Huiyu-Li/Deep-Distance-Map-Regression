...
def train_valid_seg(episode,config):
    ...
    if config['if_test']:
        ...
        test_diceLoss, test_smoothL1, test_loss, test_dice1, test_dice2, test_iter  = test(test_loader, net, net_tail, diceLoss, smoothL1, config['saved_dir'])
    ...
    for epoch in range(config['max_epoches']):
        for epi in range(episode):
            ...
            train_diceLoss, train_smoothL1, train_loss, train_dice1, train_dice2, train_iter = train(train_loader, net, net_tail, diceLoss, smoothL1, optimizer)
            ...
            valid_diceLoss, valid_smoothL1, valid_loss, valid_dice1, valid_dice2, valid_iter = validate(val_loader, net, net_tail, diceLoss, smoothL1)
...
def train(data_loader, net, net_tail, diceLoss, smoothL1, optimizer):
    ...
    for i, (data,target,target_map,origin,direction,space,prefix,subNo) in enumerate(data_loader):
        ...
        loss = loss_dice+loss_smoothL1*10
...
def validate(data_loader, net, net_tail, diceLoss, smoothL1):
    net.eval()
    epoch_diceLoss = []
    epoch_smoothL1 = []
    epoch_loss = []
...
if __name__ == '__main__':
	...
	viz = Visdom(env='G2 TumorNet-MapDice cascade frozen-MapNet-SmoothL1*10')
	...
	from TumorNet import TumorNet, Dice, MapDiceLoss
	from MapNet import *
