...
def train_valid_seg(episode,config):
    ...
    if config['if_test']:
        ...
        test_smoothL1, test_dice1, test_dice2, test_iter  = test(test_loader, net, net_tail, smoothL1, config['saved_dir'])
        test_avgsmoothL1 = sum(test_smoothL1) / test_iter
        test_avgdice1 = sum(test_dice1) / test_iter
        test_avgdice2 = sum(test_dice2) / test_iter
        print("test_smoothL1:%.4f, test_dice1:%.4f, test_dice2:%.4f, Time:%.3fmin" %
              (test_avgsmoothL1, test_avgdice1, test_avgdice2, (time.time() - start_time) / 60))
        return
    ...
    for epoch in range(config['max_epoches']):
        for epi in range(episode):
            ...
            train_smoothL1, train_dice1, train_dice2, train_iter = train(train_loader, net, net_tail, smoothL1, optimizer)
            ...
            print("[%d-%d/%d], train_smoothL1:%.4f, train_dice1:%.4f, train_dice2:%.4f, Time:%.3fmin" %
                  (epoch, epi, config['max_epoches']-1, train_avgsmoothL1, train_avgdice1, train_avgdice2,(time.time() - start_time) / 60))

            print('######valid epoch-epi', str(epoch),'-',str(epi),'######')
            valid_smoothL1, valid_dice1, valid_dice2, valid_iter = validate(val_loader, net, net_tail, smoothL1)
            ...
            scheduler.step(valid_avgsmoothL1)
            print("[%d-%d/%d], valid_smoothL1:%.4f, valid_dice1:%.4f, valid_dice2:%.4f,Time:%.3fmin " %
                  (epoch, epi, config['max_epoches'], valid_avgsmoothL1, valid_avgdice1, valid_avgdice2,(time.time() - start_time) / 60))

            #if-save-model:
            if train_avgsmoothL1 < min_loss:
                min_loss = train_avgsmoothL1
...
def train(data_loader, net, net_tail, smoothL1, optimizer):
    ...
    for i, (data,target,target_map,origin,direction,space,prefix,subNo) in enumerate(data_loader):
        ...
        loss_smoothL1.backward()
        optimizer.step()

        num = len(prefix)
        name = [prefix[i].split('-')[-1] + '/' + subNo[i] for i in range(num)]
        print("[%d/%d], loss_smoothL1:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
              (i, total_iter, loss_smoothL1.item(), dice1, dice2, name))
    return epoch_smoothL1, epoch_dice1, epoch_dice2, total_iter

def validate(data_loader, net, net_tail, smoothL1):
    net.eval()
    epoch_smoothL1 = []
    epoch_dice1 = []
    epoch_dice2 = []
...
def test(data_loader, net, net_tail, smoothL1, saved_dir):
    net.eval()
    epoch_smoothL1 = []
    epoch_dice1 = []
    epoch_dice2 = []
    ...
    with torch.no_grad():  # no backward
        for i, (data, target, target_map,origin, direction, space, prefix, subNo) in enumerate(data_loader):
            ...
            name = [prefix[i].split('-')[-1] + '/' + subNo[i] for i in range(num)]
            print(
                "[%d/%d], loss_smoothL1:%.4f, dice1:%.4f, dice2:%.4f, name:%s" %
                (i, total_iter, loss_smoothL1.item(), dice1, dice2, name))
...
if __name__ == '__main__':
	...
	viz = Visdom(env='G6 TumorNet cascade frozen-MapNet-SmoothL1 tumor')
	viz.line([0], [0], win='tloss',opts=dict(title='train avgloss', legend=['smoothL1']))
	viz.line([0], [0], win='tdice', opts=dict(title='train avgdice', legend=['dice1', 'dice2']))
	viz.line([0], [0], win='vloss',opts=dict(title='valid avgloss', legend=['smoothL1']))
	...
	config = {
		...
		'ckpt_dir': './Gt00results/',
		'saved_dir': "/data/lihuiyu/LiTS/Valid_results/check822SL11N/",#"/data/lihuiyu/LiTS/Valid_results/checkDiceCosine0/",
        'model_dir':"/home/lihuiyu/Code/LiTS_TumorNet_Map/G00results/20200213-222003/model_2-0-0220.pth",
        ...
        'tail_dir':"/home/lihuiyu/Code/LiTS_TumorNet_Map/G00results/20200213-222003/tail_2-0-3761.pth"
            #"/home/lihuiyu/Code/LiTS_TumorNet_Map/STOATumorMapNet/SignNormInverse/20200109-193132/model_14-0-0159.pth"
        # 'model_dir': ,
        # 'tail_dir':
	}
	##########hyperparameters##########
